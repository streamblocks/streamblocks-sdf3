 ...
 *
 * $Id: generate.cc,v 1.7.2.1 2010-04-25 00:10:19 mgeilen Exp $
 *
...
namespace SDF
{
        ...
        while (g->nrActors() < nrActors)
        {
            ...
            if (g->nrActors() > 1)
            {
                ...
                if (mtRand.rand() < 0.5)
                {
                    ...
                    piIter = randomPort(inPortsActor.begin(), (uint)inPortsActor.size());
                    pi = *piIter;

                    // Select random output port on set of output ports
                    poIter = randomPort(outPorts.begin(), (uint) outPorts.size());
                    po = *poIter;
                ...
                else
                {
                    ...
                    poIter = randomPort(outPortsActor.begin(), (uint) outPortsActor.size());
                    po = *poIter;

                    // Select random input port on set of input ports
                    piIter = randomPort(inPorts.begin(), (uint) inPorts.size());
                    pi = *piIter;
    ...
    void makeConsistentConnectedActors(CFractions &fractions, SDFactor *a)
    {
        ...
        for (SDFportsIter iter = a->portsBegin(); iter != a->portsEnd(); iter++)
        {
                ...
                && fractionB != knownFractionB)
            {
                ...
                pB->setRate((SDFrate) ratioAB.numerator());
                pA->setRate((SDFrate) ratioAB.denominator());
            }
    ...
    SDFactor *getRandomActorFromActorReadyList(SDFactors &readyList)
    {
        ...
        n = mtRand.randInt((MTRand::uint32)readyList.size() - 1);
