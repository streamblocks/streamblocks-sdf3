 ...
 *
 * $Id: sdf3generate.cc,v 1.4 2008-10-16 14:44:29 sander Exp $
 *
...
namespace SDF
{
    ...
    void parseSettingsFile(CString module, CString type)
    {
        ...
        CNode *bandwidthNode, *latencyNode, *settingsNode;
        CNode *sdf3Node = NULL;
        CString name, file;
        CDoc *settingsDoc;
        ...
        if (settingsDoc != NULL) sdf3Node = CGetRootNode(settingsDoc);
        if (sdf3Node == NULL)
        {
        ...
        if (actorsNode == NULL)
            throw CException("Missing actors element in settings.");
        degreeNode = CGetChildNode(graphNode, "degree");
        if (degreeNode == NULL)
            throw CException("Missing degree element in settings.");
        rateNode = CGetChildNode(graphNode, "rate");
        if (rateNode == NULL)
            throw CException("Missing rate element in settings.");
