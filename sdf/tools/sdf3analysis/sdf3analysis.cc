...
namespace SDF
{
    ...
    void helpMessage(ostream &out)
    {
        ...
        out << "       mpthroughput" << endl;
        out << "       mpdelay" << endl;
        out << "       mpeigen" << endl;
        out << "       mpconverthsdf" << endl;
        out << "       is_hsdf" << endl;
        out << "       statistics" << endl;
        out << "       throughput" << endl;
        out << "       buffersize" << endl;
    ...
    void analyzeSDFG(TimedSDFgraph *g, CPairs &analyze, ostream &out)
    {
        ...
        else if (analyze.front().key == "buffersize_capacity_constrained")
        {
            ...
            /// @todo
            /// what is a reasonable precision value?
            /// is lowestTerm necessary after doubleToFraction?
            CFraction mcmf;
            mcmf.doubleToFraction(mcm, 1e-5);
            mcmf = mcmf.lowestTerm();

            // MCM must be an integer value
            if (mcmf.denominator() != 1)
        ...
        else if (analyze.front().key == "latency")
        {
            ...
            method = options.front();
            options.pop_front();
            srcActorName = options.front();
            options.pop_front();
            dstActorName = options.front();
            options.pop_front();
        ...
        else if (analyze.front().key == "mpthroughput")
        {
            double thr;
        ...
        else if (analyze.front().key == "mpdelay")
        {
            double d;
            ...
            while (channelNode != nullptr)
            {
                ...
                while (tokenNode != nullptr)
                {
                    ...
                    tokenNode = CNextNode(tokenNode, "token");
                }
            ...
            MaxPlus::Vector *gamma_s = new MaxPlus::Vector(gamma_s_v);
            delete gamma_s_v;
            ...
            while (mpschedule_initNode != nullptr)
            {
                ...
                MaxPlus::Vector *gamma0 = new MaxPlus::Vector(gamma0_v);
                delete gamma0_v;

                // Measure execution time
                startTimer(&timer);
                d = mpe.exploreDelayForSchedule(gamma0, gamma_s, period);
                ...
                mpschedule_initNode = CNextNode(mpschedule_initNode, "mpschedule_initial");
                delete gamma0;
            }
        }
        else if (analyze.front().key == "mpconverthsdf")
        {
            GraphDecoration::Graph *G = new GraphDecoration::Graph(g);
            MaxPlusAnalysis::Exploration mpe;
            mpe.G = G;
            ...
            // Measure execution time
            startTimer(&timer);
            TimedSDFgraph *h = mpe.convertToHSDFByMP();

            // Measure execution time
            stopTimer(&timer);
            outputSDFasXML(h, out);
        ...
        else if (analyze.front().key == "mpeigen")
        {
            ...
            out << "eigenvalue: " << eigenvalue << endl;

            unsigned int i;
            for (i = 0; i < eigenvector->getSize(); i++)
            {
                out << eigenvector->get(i) << " ; ";
            }
