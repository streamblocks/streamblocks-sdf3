 ...
 *
 * $Id: binding_aware_sdfg.cc,v 1.3.2.1 2010-04-25 02:08:51 mgeilen Exp $
 *
...
namespace SDF
{
            ...
            PlatformGraph *platformGraph)
    {
        ...
        nrTiles = (uint) platformGraph->nrTiles();
             ...
             iter != platformGraph->tilesEnd(); iter++)
        {
            ...
            if (p != NULL)
            {
                ...
                // TDMA wheel size
                tdmaSize[t->getId()] = (CId) p->getTimewheelSize();

                // TDMA time slice of application
                tdmaSlice[t->getId()] = (CId) p->getReservedTimeSlice();
            }
        }
    }
    ...
    void BindingAwareSDFG::createMappedActorNSoC(TimedSDFactor *a, Tile *t)
    {
        ...
        a->getGraph()->createChannel(a, 1, a, 1, 1);
    }
            ...
            Connection *cn)
    {
        ...
        latency.execTime = (SDFtime)(cn->getLatency() + (TTime)(ceil(connectionDelay)));
        ...
        if (dstP->getTimewheelSize() > dstP->getReservedTimeSlice())
        {
            ...
            tdma.type = "tdma";
            tdma.execTime = (SDFtime)(dstP->getTimewheelSize() - dstP->getReservedTimeSlice());
            tdma.stateSize = 0;
            d->addProcessor(&tdma);
            d->setDefaultProcessor("tdma");

            // The channel ch must be split in 3 channels connected to actor c and d
            g->createChannel(c, 1, d, 1, 0);
            g->createChannel(d, 1, ch->getDstActor(),
                             ch->getDstPort()->getRate(), 0);
        }
        else
        {
            g->createChannel(c, 1, ch->getDstActor(),
                             ch->getDstPort()->getRate(), 0);
        }
        ...
        if (ch->getMinLatency() > 0)
        {
            ...
            (TimedSDFchannel *) g->createChannel(e, 1, ch->getDstActor(),
                                                 ch->getDstPort()->getRate(), 0);
        ...
        TimedSDFchannel *ch, Connection *cn)
    {
        ...
        TimedSDFchannel *chSrcSemaphoreActor;
        ...
        tdmaSrcSync.execTime = (SDFtime)(srcProc->getTimewheelSize()
        ...
        tdmaDstSync.execTime = (SDFtime)(dstProc->getTimewheelSize()
        ...
        (TimedSDFchannel *) g->createChannel(
